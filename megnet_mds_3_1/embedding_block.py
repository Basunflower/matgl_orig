...
class EmbeddingBlock(nn.Module):
    ...
    def __init__(
        ...
        self.orig_elem_list = orig_elem_list  # 用于替换原始的序列
        ...
        self.fix_embed = nn.Embedding.from_pretrained(torch.tensor(pretrain_embeddings), freeze=True)
    ...
    def forward(self, node_attr, edge_attr, state_attr):
        ...
        """
        if self.ntypes_node is not None:  # 修改为固定embedding
            # 处理node_attr转化为原子序数
            Z_node_attr = self.Init_to_Z(node_attr)
            Z_node_attr = torch.tensor(Z_node_attr, device=node_attr.device)
            node_feat = self.fix_embed(torch.tensor(Z_node_attr))
    ...
    def Init_to_Z(self, node_attr):
        elem_list = self.orig_elem_list
        # 转化为原子序数的字典
        atomic_numbers = {element: Element(element).Z for element in elem_list}
        # 将化合物转化为原子序数
        Z_node_attr = [atomic_numbers[elem_list[index]] for index in node_attr]
        return Z_node_attr
